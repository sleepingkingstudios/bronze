# spec/patina/operations/entities/update_one_operation_spec.rb

require 'patina/collections/simple'
require 'patina/operations/entities/entity_operation_examples'
require 'patina/operations/entities/update_one_operation'

require 'support/example_entity'

RSpec.describe Patina::Operations::Entities::UpdateOneOperation do
  ...
  describe '#call' do
    shared_examples 'should return false and set the errors' do
      ...
      it 'should not update the record in the collection' do
        expect { instance.call resource }.
          not_to change { collection.find(resource.id) }
      end # it
    ...
    shared_examples 'should update the resource and return true' do
      it { expect(instance.call resource).to be true }

      it 'should update the record in the collection' do
        expect { instance.call resource }.
          to change { collection.find(resource.id) }.
          to be == resource
      end # it
    ...
    wrap_context 'when the collection contains many resources' do
      ...
      describe 'with a resource that is in the repository' do
        let(:resource) do
          resources.first.tap { |res| res.title = 'Your Daily Horoscope' }
        end # let

        include_examples 'should update the resource and return true'
      end # describe
    end # wrap_context
  end # describe
